...
Review all files in an epic directory for quality, consistency, and execution
readiness.
...
Your developer wrote up this epic plan. Review all files in the epic directory
and subdirectories.
...
This command performs a comprehensive review of ALL files in the epic directory
(spec, epic YAML, tickets, and any other generated files) to validate the epic
is ready for execution. It assesses:

1. **Consistency across planning documents** - Do the spec, epic YAML, and
   tickets align?
2. **Implementation completeness** - Will implementing the tickets as described
   produce the functionality described in the spec?
3. **Test coverage gaps** - Are there areas described in the spec that lack
   corresponding test requirements in tickets?

The review identifies quality issues, missing information, inconsistencies, and
...
- Are required sections present (Description, Dependencies, Acceptance
  Criteria, etc.)?

### 6. Ticket Description Quality
...
### 9. Epic YAML Coordination Quality
...
### 11. Consistency Across Tickets

- Do tickets use consistent terminology?
- Are shared concepts (like data models, interfaces) referenced
  consistently?
- Do tickets that should coordinate with each other align properly?
- Are naming conventions consistent (function names, class names, file
  paths)?

### 12. Implementation Clarity

- Is it clear what code needs to be written?
- Are there ambiguous requirements that could be interpreted multiple
  ways?
- Are there missing specifications (error handling, validation, edge
  cases)?
...
1. **Read the spec file** (`*-spec.md`) to understand what functionality needs
   to be built
2. **Read the epic YAML file** to understand architecture, coordination
...
6. **Check implementation completeness** - Will tickets build what the spec
   describes?
...
10. **Create the artifacts directory** if it doesn't exist (e.g.,
    `.epics/[epic-name]/artifacts/`)
11. **Write findings** to `.epics/[epic-name]/artifacts/epic-review.md` using
    the Write tool

## Output Format

Your review should be written to `.epics/[epic-name]/artifacts/epic-review.md`
with this structure:
...
[2-3 sentences: Is this epic ready for execution? High-level quality
assessment.]

## Consistency Assessment

[Do the spec, epic YAML, and tickets align? Are there contradictions or
mismatches?]

## Implementation Completeness

[Will implementing the tickets produce what the spec describes? What's missing
or extra?]
...
[Prioritized list of improvements: Priority 1 (must fix), Priority 2 (should
fix), Priority 3 (nice to have)]
```

**Note:** Session IDs (`builder_session_id` and `reviewer_session_id`) will be
